...
function getTemplates(options, uiOptions, nuxt) {
  ...
  function writeThemeTemplate(theme2, path) {
    for (const component in theme2) {
      templates.push({
        filename: `ui-pro/${path ? path + '/' : ''}${scule.kebabCase(
          component,
        )}.ts`,
        write: true,
        getContents: async () => {
    ...
    templates.push({
      ...
      getContents: () =>
        Object.keys(themeProse)
          .map(
            (component) =>
              `export { default as ${component} } from './${scule.kebabCase(
                component,
              )}'`,
          )
          .join('\n'),
  ...
  if (!!nuxt && (kit.hasNuxtModule('@nuxt/content') || options.content)) {
    hasContent = true;
    ...
    getContents: () => {
      ...
      if (hasContent) {
        ...
        contents += Object.keys(themeContent)
          .map(
            (component) =>
              `export { default as ${component} } from './content/${scule.kebabCase(
                component,
              )}'`,
          )
          .join('\n');
